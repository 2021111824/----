...
def initialize_population(n, server_count, P, user_positions, server_positions, priorities,
    ...
    for _ in tqdm(range(P), desc="Initializing Population"):
        ...
        while not valid_individual:
            ...
            # 检查是否满足所有约束
            valid, penalty = check_constraints(individual, user_positions, server_positions, priorities,
                                                 R_cpu, R_mem, R_bandwidth, cpu_demands, mem_demands, bandwidth_demands,
                                                 cost_edge, cost_cloud, m_edge, max_cost, T_max, request_sizes,
                                                 v_edge, v_cloud, b_edge, b_cloud, P_edge, P_cloud, p_net)
...
def compute_fitness_new(individual, user_positions, server_positions, request_sizes, priorities, weights,
    ...
    # 计算Jain公平性指数
    jain_fairness_sum = 0
    for priority in response_stats.keys():
        # 获取当前优先级内所有用户的响应时间
        ...
        # 将Jain公平性指数加到总和中
        jain_fairness_sum += (1 - jain_fairness_index)  # 我们希望最小化 1 - F_jain

    # 使用λ1和λ2加权目标函数
    lambda_1 = 0.5
    lambda_2 = 0.5

    fitness = 1/(lambda_1 * total_response_deviation + lambda_2 * jain_fairness_sum)

    return fitness, total_response_deviation, jain_fairness_sum
...
def compute_jain_fairness(response_times):
    ...
    total_square_sum = sum(t ** 2 for t in response_times)

    # 计算Jain公平性指数
    F_jain = (total_sum ** 2) / (n * total_square_sum)
    return F_jain
...
def compute_fitness(individual, user_positions, server_positions, request_sizes, priorities, weights,
    ...
    if not valid:
        ...
        # 重新检查修复后的约束
        valid, penalty = check_constraints(
    ...
    # 应用惩罚
    fitness = F_jain - penalty  # 惩罚扣除适应度
    return fitness
...
def genetic_algorithm(user_positions, server_positions, request_sizes, priorities, weights,
    ...
    # 用来记录每代的最优Jain公平性指数和响应时间比例偏差
    best_jain_fairness_history = []
    best_response_deviation_history = []
    ...
    for g in tqdm(range(G_max), desc="Running Genetic Algorithm"):
        ...
        for ind in population:
            fitness, response_deviation, jain_fairness_sum = compute_fitness_new(ind, user_positions, server_positions,
                                                                                 request_sizes, priorities, weights,
                                                                                 v_edge, v_cloud, b_edge, b_cloud,
                                                                                 P_edge, P_cloud, R_cpu, R_mem, R_bandwidth,
                                                                                 cost_edge, cost_cloud, m_edge, max_cost,
                                                                                 cpu_demands, mem_demands,
                                                                                 bandwidth_demands, p_net, T_max)

            fitnesses.append(fitness)
            response_deviations.append(response_deviation)
            jain_fairness_sums.append(jain_fairness_sum)
        ...
        # 记录最优的Jain公平性和响应时间偏差
        best_jain_fairness_history.append(1 - jain_fairness_sums[best_idx])  # 记录1 - F_jain
        best_response_deviation_history.append(response_deviations[best_idx])  # 记录响应时间偏差
        ...
        for idx, ind in enumerate(new_population[1:]):  # 对新种群进行变异
            ...
            # 修复不满足约束的个体
            valid, penalty = check_constraints(ind, user_positions, server_positions, priorities,
                                     R_cpu, R_mem, R_bandwidth, cpu_demands, mem_demands, bandwidth_demands,
                                     cost_edge, cost_cloud, m_edge, max_cost, T_max, request_sizes,
                                     v_edge, v_cloud, b_edge, b_cloud, P_edge, P_cloud, p_net)
            if not valid:
    ...
    return best_solution, best_fitness, fitness_history, best_jain_fairness_history, best_response_deviation_history  # 返回最优解及其适应度值。
