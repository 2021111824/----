...
def check_constraints(individual, user_positions, server_positions, priorities,
    ...
    # 检查约束：低优先级用户的平均响应时间是相邻高优先级用户的平均响应时间的1.2-1.5倍
    if valid:  # 只有在其他约束满足的情况下才检查软约束
        for i in range(1, len(sorted_priorities)):  # i为 1 到 len(priorities)-1
            low_priority = i
            high_priority = i + 1
            ...
            if not (high_priority_mean * 1.2 <= low_priority_mean <= high_priority_mean * 1.5):
                # 如果不满足约束，则计算惩罚并继续
                penalty += abs((low_priority_mean / high_priority_mean) - 1.35) * 0.5  # 惩罚因子，理想值1.35
