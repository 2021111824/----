...
def compute_fitness_new(individual, user_positions, server_positions, request_sizes, priorities,
    ...
    for i in range(n_users):
        ...
        response_times.append(Decimal(str(response_time)))  # 转换为 Decimal 对象

    # 使用 calculate_response_stats 计算不同优先级的响应时间统计信息
    response_stats = calculate_response_stats([float(t) for t in response_times],
                                              priorities)  # 这里可能需要调整 calculate_response_stats 函数支持 Decimal
    sorted_priorities = sorted(response_stats.keys(), reverse=False)  # 按优先级从低到高排序

    # 计算响应时间比例偏差
    total_response_deviation = Decimal('0')

    for i in range(1, len(sorted_priorities)):
        low_priority = i
        high_priority = i + 1

        low_priority_mean = Decimal(str(response_stats[low_priority]["mean"]))
        high_priority_mean = Decimal(str(response_stats[high_priority]["mean"]))

        r_i = Decimal('1.35')  # 获取期望的响应时间比例 r_i
    ...
    fitness = -(lambda_1 * total_response_deviation +
                lambda_2 * sum(
                (Decimal('1') - jain_fairness_index) for jain_fairness_index in jain_fairness_indices.values()))

    return float(fitness), float(total_response_deviation), {k: float(v) for k, v in jain_fairness_indices.items()}
...
def compute_jain_fairness(response_times):
    ...
    total_sum = sum(Decimal(str(t)) for t in response_times)
    total_square_sum = sum(Decimal(str(t)) ** 2 for t in response_times)

    # 计算Jain公平性指数
    F_jain = (total_sum ** 2) / (Decimal(str(n)) * total_square_sum)
    return F_jain
...
def genetic_algorithm(user_positions, server_positions, request_sizes, priorities,
    ...
    for g in tqdm(range(G_max), desc="Running Genetic Algorithm"):
        ...
        for ind in population:
            fitness, response_deviation, jain_fairness_indic = compute_fitness_new(ind, user_positions, server_positions,
    ...
    # 返回最优解及其适应度值
    return best_solution, best_fitness, best_jain_fairness, best_response_deviation, \
