...
from genetic_algorithm import compute_fitness_new
random.seed(42)
np.random.seed(42)
# ========== 主程序入口 ==========
if __name__ == "__main__":
    ...
    # ========== 遗传算法求解 ==========
    best_solution, best_fitness, jain_fairness, response_deviation, fitness_history, best_jain_fairness_history, best_response_deviation_history = genetic_algorithm(
    ...
    # ========== 结果分析 ==========
    # 根据分配情况计算每个用户的计算能力
    user_capacities = assign_computational_capacity(best_solution, user_positions, server_positions, request_sizes, P_edge,
    ...
    fitness, total_response_deviation, jain_fairness_indices = compute_fitness_new(best_solution, user_positions, server_positions, request_sizes, priorities,
    ...
    #     # 获取当前优先级内所有用户的响应时间
    #     priority_users_response_times = [response_times[i] for i in range(n) if priorities[i] == priority]
    #
    #     # 计算该优先级内的Jain公平性指数
    ...
    print("测试Jain计算情况")
    for priority, fairness in jain_fairness_indices.items():
        print(f"  Priority {priority}: {fairness:.4f}\n")  # 格式化每个值

    print("测试比例值偏差")
    print(total_response_deviation)
