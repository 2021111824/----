...
def plot_fitness_history(fitness_history, output_folder):
    ...
    plt.legend()
    plt.grid(alpha=0.5)
    plt.savefig(os.path.join(output_folder, "history_of_fitness.png"))
    plt.show()


def plot_jain_history(best_jain_fairness_history, output_folder):
    """
    画出不同优先级的 Jain 公平性指数随代数变化的折线图
    """
    generations = list(range(len(best_jain_fairness_history)))  # 生成代数编号
    priority_levels = best_jain_fairness_history[0].keys()  # 获取所有优先级

    plt.figure(figsize=(10, 6))

    for priority in priority_levels:
        fairness_values = [history[priority] for history in best_jain_fairness_history]
        plt.plot(generations, fairness_values, label=f"Priority {priority}")

    plt.xlabel("Generation")
    plt.ylabel("Jain Fairness Index")
    plt.title("Jain Fairness Index Evolution Across Generations")
    plt.legend()
    plt.grid(True)

    # 保存图像
    plt.savefig(os.path.join(output_folder, "history_of_jain_fairness.png"))
    plt.show()
...
def plot_response_deviation_history(best_response_deviation_history, output_folder):
    ...
    plt.legend()
    plt.grid(alpha=0.5)
    plt.savefig(os.path.join(output_folder, "history_of_response_deviation.png"))
    plt.show()
