...
def genetic_algorithm(user_positions, server_positions, request_sizes, priorities,
    ...
    for g in tqdm(range(G_max), desc="Running Genetic Algorithm"):
        ...
        if fitnesses[best_idx] > best_fitness:  # 更新全局最优
            ...
            jain_fairness = jain_fairness_indices[best_idx]
            response_deviation = response_deviations[best_idx]

        fitness_history.append(best_fitness)  # 记录每代的最优适应度
        # 记录最优的Jain公平性和响应时间偏差
        best_jain_fairness_history.append(jain_fairness)  # 记录1 - F_jain
        best_response_deviation_history.append(response_deviation)  # 记录响应时间偏差
    ...
    # 返回最优解及其适应度值
    return best_solution, best_fitness, jain_fairness, response_deviation, fitness_history, best_jain_fairness_history, best_response_deviation_history
