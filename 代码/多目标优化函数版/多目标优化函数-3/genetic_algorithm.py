...
def compute_fitness_new(individual, user_positions, server_positions, request_sizes, priorities, weights,
    ...
    fitness = -(lambda_1 * total_response_deviation +
                   lambda_2 * sum(jain_fairness_index for jain_fairness_index in jain_fairness_indices.values()))

    return fitness, total_response_deviation, jain_fairness_indices
...
def genetic_algorithm(user_positions, server_positions, request_sizes, priorities, weights,
    ...
    for g in tqdm(range(G_max), desc="Running Genetic Algorithm"):
        ...
        for ind in population:
            fitness, response_deviation, jain_fairness_indice = compute_fitness_new(ind, user_positions, server_positions,
            ...
            jain_fairness_indices.append(jain_fairness_indice)
        ...
        # 记录最优的Jain公平性和响应时间偏差
        best_jain_fairness_history.append(jain_fairness_indices[best_idx])  # 记录1 - F_jain
