...
# 1.2 贪心算法初始化种群
def greedy_initialize_population(n, server_count, P, user_positions, server_positions, priorities,
                                 R_cpu, R_mem, R_bandwidth, cpu_demands, mem_demands, bandwidth_demands,
                                 cost_edge, cost_cloud, m_edge, max_cost, T_max, request_sizes,
                                 v_edge, v_cloud, b_edge, b_cloud, P_edge, P_cloud, p_net):
    """
        利用贪心算法初始化种群，连接到满足资源约束且响应时间最短的服务器
    ...
    population = []
    for _ in tqdm(range(P), desc="Greedy Initializing Population"):
        individual = np.zeros((n, server_count), dtype=int)
        # 初始化服务器资源使用情况
        server_cpu_usage = np.zeros(server_count)
        server_mem_usage = np.zeros(server_count)
        ...
        for i in range(n):
            ...
            # 提前筛选出可能满足条件的服务器
            possible_servers = [j for j in range(server_count) if
                                server_cpu_usage[j] + cpu_demands[i] <= R_cpu[j] and
                                server_mem_usage[j] + mem_demands[i] <= R_mem[j] and
                                server_bandwidth_usage[j] + bandwidth_demands[i] <= R_bandwidth[j]]
            for j in possible_servers:
                is_edge = j < m_edge
                user_capacities = assign_computational_capacity(individual, user_positions, server_positions,
                                                                request_sizes, P_edge, P_cloud, m_edge, priorities)
                response_time = compute_response_time(
                    user_positions[i], server_positions[j], is_edge, request_sizes[i], user_capacities[i],
                    v_edge, v_cloud, b_edge, b_cloud
                )
                if response_time < best_response_time:
                    best_response_time = response_time
                    best_server_idx = j
            if best_server_idx is not None:
                individual[i, best_server_idx] = 1
                server_cpu_usage[best_server_idx] += cpu_demands[i]
                server_mem_usage[best_server_idx] += mem_demands[i]
                server_bandwidth_usage[best_server_idx] += bandwidth_demands[i]
        valid = check_constraints(individual, user_positions, server_positions, priorities,
                                  R_cpu, R_mem, R_bandwidth, cpu_demands, mem_demands, bandwidth_demands,
                                  cost_edge, cost_cloud, m_edge, max_cost, T_max, request_sizes,
                                  v_edge, v_cloud, b_edge, b_cloud, P_edge, P_cloud, p_net)
        if valid:
            ...
            print("Warning: Generated individual does not satisfy constraints. Skipping.")
    return population
...
def genetic_algorithm(user_positions, server_positions, request_sizes, priorities,
    ...
    遗传算法主函数。
    """
    population = greedy_initialize_population(len(user_positions), len(server_positions), P,  user_positions, server_positions, priorities,
                                       R_cpu, R_mem, R_bandwidth, cpu_demands, mem_demands, bandwidth_demands,
                                       cost_edge, cost_cloud, m_edge, max_cost, T_max, request_sizes,
                                       v_edge, v_cloud, b_edge, b_cloud, P_edge, P_cloud, p_net)
