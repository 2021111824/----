...
if __name__ == "__main__":
    ...
    P_edge, P_cloud = 200, 800  # 边缘和云服务器的计算能力 (MB/s)
    P = 200  # 初始种群大小
    G_max = 100  # 最大迭代代数
    ...
    p_net = 0.5  # 网络流量单位成本
    max_cost = 700  # 最大允许总成本

    # 输出文件夹
    output_folder = "un_fairness_simulation_results"
    ...
    for i in range(len(user_positions)):
        ...
        response_time = compute_response_time(
            ...
            request_sizes[i], v_edge, v_cloud, b_edge, b_cloud, P_edge, P_cloud
        )
        response_times.append(response_time)
        server_resources[server_idx] += request_sizes[i]  # 累加服务器资源使用量
    ...
    print("un_fairness_Response Time Statistics:")
    ...
    # 保存到文件
    with open(os.path.join(output_folder, "un_fairness_cost_results.txt"), "w") as f:
        f.write("===== un_fairness_Cost Results =====\n")
    ...
    # 保存详细结果到文件
    with open(os.path.join(output_folder, "un_fairness_simulation_results.txt"), "w") as f:
        f.write("===== un_fairness_Simulation Results =====\n")
        ...
        for i, usage in enumerate(server_resources):
