...
def initialize_topology(n, m_edge, m_cloud):
    ...
    # 优先级权重
    priority_weights = {1: 1, 2: 2, 3: 4}  # 优先级到权重的映射
    ...
    cpu_demands = np.random.uniform(0.1, 0.2, n) * request_sizes  # 假设每MB请求需要0.1~0.2个CPU
    cpu_demands = cpu_demands * levels

    # 根据请求大小计算内存需求 (单位：GB)
    mem_demands = np.random.uniform(0.02, 0.05, n) * request_sizes  # 假设每MB请求需要0.01~0.05GB内存
