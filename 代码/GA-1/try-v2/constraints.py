...
from calculations import calculate_total_cost, compute_response_time, calculate_response_stats
...
def check_constraints(individual, user_positions, server_positions, priorities,
                      ...
                      cost_edge, cost_cloud, m_cloud, m_edge, max_cost, T_max, request_sizes,
    ...
    for i in range(len(user_positions)):
        ...
        response_time = compute_response_time(
            ...
            v_edge, v_cloud, b_edge, b_cloud, individual,
            cpu_demands, mem_demands, bandwidth_demands, R_cpu, R_mem, R_bandwidth, m_edge, m_cloud, user_positions
        )
    ...
    for i in range(n_users):
        server_idx = np.argmax(individual[i])  # 用户分配到的服务器索引
    ...
    for j in range(n_servers):
        if server_cpu_usage[j] > R_cpu[j]:
            return False
        if server_mem_usage[j] > R_mem[j]:
            return False
