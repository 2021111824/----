...
import numpy as np
import random
from calculations import compute_response_time, assign_computational_capacity
...
def mutate(individual, user_positions, server_positions, m_edge, P_m, priorities):
    """
    对个体进行变异，优先级高的用户变异到边缘服务器，且优先级越高，变异到边缘服务器的概率越大。
    ...
    for i in range(len(individual)):
        # 如果随机值小于总的变异概率P_m，则发生变异
        if random.random() < P_m:
            if priorities[i] == 3:  # 优先级为3的用户，尽量变异到边缘服务器
                # 找到离用户最近的边缘服务器
                random_edge_server_idx = random.randint(0, m_edge - 1)
                individual[i] = 0  # 重置用户分配
                individual[i, random_edge_server_idx] = 1  # 将用户分配到最近的边缘服务器
            elif priorities[i] == 2:  # 优先级为2的用户
                if random.random() < 0.5:  # 50%的概率变异到边缘服务器
                    random_edge_server_idx = random.randint(0, m_edge - 1)
                    individual[i] = 0  # 重置用户分配
                    individual[i, random_edge_server_idx] = 1  # 将用户分配到最近的边缘服务器
                else:  # 50%的概率随机选择服务器
                    server_idx = random.randint(0, len(server_positions) - 1)
                    individual[i] = 0  # 重置用户分配
                    individual[i, server_idx] = 1  # 将用户分配到随机服务器
            elif priorities[i] == 1:  # 优先级为1的用户
                if random.random() < 0.3:  # 30%的概率变异到边缘服务器
                    random_edge_server_idx = random.randint(0, m_edge - 1)
                    individual[i] = 0  # 重置用户分配
                    individual[i, random_edge_server_idx] = 1  # 将用户分配到最近的边缘服务器
                else:  # 70%的概率随机选择服务器
