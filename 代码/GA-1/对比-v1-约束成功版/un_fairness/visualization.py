...
def plot_fitness_history(fitness_history, output_folder):
    ...
    """
    plt.figure(figsize=(10, 6))
    plt.plot(range(len(fitness_history)), fitness_history, label="un_fairness_Total Response Time")
    plt.title("Fitness Over Generations (Minimize Total Response Time)")
...
def plot_response_time_distribution(response_times, priorities, output_folder):
    ...
    for level in np.unique(priorities):  # 遍历每种优先级
        ...
        plt.hist(times, bins=20, alpha=0.6, label=f"Priority {level}", color=colors[level - 1])

    plt.title("un_fairness_Response Time Distribution by Priority")
    ...
    # 绘制整体响应时间分布
    plt.figure(figsize=(10, 6))
    plt.hist(response_times, bins=20, color='gray', edgecolor='black', alpha=0.7)
    plt.title("un_fairness_Overall Response Time Distribution")
    plt.xlabel("Response Time (ms)")
    plt.ylabel("Number of Users")
    plt.grid(alpha=0.5)
    plt.savefig(os.path.join(output_folder, "un_fairness_response_time_distribution_overall.png"))
    plt.show()
...
def plot_avg_response_time(response_times, priorities, output_folder, T_max):
    ...
    # 图表标题和标签
    plt.title("un_fairness_Average Response Time by Priority")
    plt.xlabel("Priority Level")
    plt.ylabel("Average Response Time (ms)")
    plt.grid(alpha=0.5)
    plt.savefig(os.path.join(output_folder, "un_fairness_avg_response_time.png"))
    plt.show()
...
def plot_server_resource_usage(server_cpu_usage, server_mem_usage, server_bandwidth_usage,
    ...
    plt.text(m_edge - 1, max_edge_cpu + 1, f"un_fairness_Edge Total CPU Usage: {edge_cpu_usage_rate:.1f}%", ha='center',
    ...
    plt.legend()
    plt.grid(alpha=0.5)
    plt.savefig(os.path.join(output_folder, "un_fairness_server_cpu_usage.png"))
    plt.show()
    ...
    plt.title("un_fairness_Server Memory Usage")
    plt.xlabel("Server Index")
    plt.ylabel("Memory Usage (GB)")
    plt.legend()
    plt.grid(alpha=0.5)
    plt.savefig(os.path.join(output_folder, "un_fairness_server_mem_usage.png"))
    plt.show()
    ...
    plt.title("un_fairness_Server Bandwidth Usage")
    plt.xlabel("Server Index")
    plt.ylabel("Bandwidth Usage (Mbps)")
    plt.legend()
    plt.grid(alpha=0.5)
    plt.savefig(os.path.join(output_folder, "un_fairness_server_bandwidth_usage.png"))
    plt.show()
...
def plot_cost_distribution(cost_details, output_folder, total_edge_cost, total_cloud_cost, total_cost, cost_limit):
    ...
    plt.title("un_fairness_Cost Breakdown by Server Type")
    plt.xticks(x, categories)  # 按固定顺序设置类别名称
    plt.legend()
    plt.grid(alpha=0.5)
    plt.tight_layout()  # 自动调整布局以适应新增内容
    plt.savefig(os.path.join(output_folder, "un_fairness_cost_distribution.png"))
    plt.show()
...
def plot_user_server_connections(user_positions, server_positions, best_solution, priorities, m_edge, output_folder):
    ...
    plt.title("un_fairness_User-to-Server Connections by Priorities")
    plt.xlabel("X Coordinate")
    plt.ylabel("Y Coordinate")
    plt.legend()
    plt.grid(alpha=0.5)
    plt.tight_layout()  # 自动调整图形布局
    plt.savefig(os.path.join(output_folder, "un_fairness_user_server_connections.png"))
    plt.show()
