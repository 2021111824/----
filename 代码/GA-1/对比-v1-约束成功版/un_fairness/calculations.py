...
def compute_response_time(user, server, is_edge, request_size, v_edge, v_cloud, b_edge, b_cloud, P_edge, P_cloud):
    """
    响应时间计算，根据是否为边缘服务器分别处理
    """
    d_ij = np.linalg.norm(user - server)  # 计算用户到服务器的距离
    if is_edge:
        return d_ij / v_edge + request_size / b_edge + request_size / P_edge  # 边缘服务器响应时间
    else:
        return d_ij / v_cloud + request_size / b_cloud + request_size / P_cloud  # 云服务器响应时间
