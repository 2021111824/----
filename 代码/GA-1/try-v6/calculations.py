...
def assign_computational_capacity(individual, user_positions, server_positions, request_sizes, P_edge, P_cloud, m_edge,
    ...
    for i, user_pos in enumerate(user_positions):
        ...
        for j in range(m_edge):  # 边缘服务器
            # 当前服务器上所有用户的请求需求总和
            total_demand_edge = np.sum(
                [request_sizes[k] * levels[k] for k in range(len(user_positions)) if np.argmax(individual[k]) == j])

            # 假设按请求大小的比例分配计算能力
            assigned_capacity = P_edge * (user_demand / total_demand_edge)  # 按该用户在边缘服务器上请求占比分配计算能力

        for j in range(m_edge, len(server_positions)):  # 云服务器
            # 当前服务器上所有用户的请求需求总和
            total_demand_cloud = np.sum(
                [request_sizes[k] * levels[k] for k in range(len(user_positions)) if np.argmax(individual[k]) == j])

            # 假设按请求大小的比例分配计算能力
            assigned_capacity = P_cloud * (user_demand / total_demand_cloud)  # 按该用户在边缘服务器上请求占比分配计算能力
        # 将分配的计算能力赋给该用户
