...
# ========== 计算逻辑 ==========
def assign_computational_capacity(user_positions, server_positions, request_sizes, P_edge, P_cloud, m_edge,
                                  priorities):
    """
    根据请求大小和优先级分配计算能力
    """
    ...
    for i, user_pos in enumerate(user_positions):
        ...
        for j in range(m_edge):  # 边缘服务器
            # 假设按请求大小的比例分配计算能力
            assigned_capacity += P_edge * (user_demand / np.sum(user_demand))  # 简单按比例分配

        for j in range(m_edge, len(server_positions)):  # 云服务器
            assigned_capacity += P_cloud * (user_demand / np.sum(user_demand))  # 简单按比例分配
