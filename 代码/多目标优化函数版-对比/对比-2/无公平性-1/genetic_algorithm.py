...
def compute_fitness(individual, user_positions, server_positions, request_sizes, priorities,
    ...
    # 目标是最小化响应时间
    return -total_response_time, total_response_deviation, jain_fairness_indices, response_times
...
def genetic_algorithm(user_positions, server_positions, request_sizes, priorities,
    ...
    for g in tqdm(range(G_max), desc="Running Genetic Algorithm"):
        ...
        for ind in population:
            fitness, response_deviation, jain_fairness_indic, response_time = compute_fitness(ind, user_positions, server_positions,
    ...
    return best_solution, best_fitness, best_jain_fairness, best_response_deviation, best_response_times,\
        fitness_history, jain_fairness_history, response_deviation_history  # 返回最优解及其适应度值。
