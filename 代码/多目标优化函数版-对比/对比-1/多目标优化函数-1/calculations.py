...
def calculate_response_stats(response_times, priorities):
    ...
    """
    # 确保response_times和priorities都是NumPy数组
    response_times = np.array(response_times)
    priorities = np.array(priorities)
    # print(response_times)
    # print(priorities)

    if len(response_times) != len(priorities):
        raise ValueError("The length of response_times must match the length of priorities.")
    ...
    for level in np.unique(priorities):  # 遍历每种优先级
        ...
        # 输出调试信息，检查idx和response_times的大小
        # print(f"idx: {idx}, len(response_times): {len(response_times)}")
