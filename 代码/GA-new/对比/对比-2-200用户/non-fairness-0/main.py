...
if __name__ == "__main__":
    ...
    T_max = {
        1: 13.5,  # 优先级 1 用户最大允许响应时间 (ms)
        2: 9,  # 优先级 2 用户最大允许响应时间 (ms)
    ...
    # ========== 数据初始化 ==========
    user_positions, request_sizes, priorities, weights, server_positions, R_cpu, R_mem, R_bandwidth, cpu_demands, \
    ...
    response_times = np.array(best_response_times)
    weighted_times = np.array([response_times[i] * weights[i] for i in range(len(response_times))])
    F_jain_2 = (np.sum(weighted_times) ** 2) / (len(weighted_times) * np.sum(weighted_times ** 2))
    ...
    with open(os.path.join(output_folder, "simulation_results.txt"), "w") as f:
        ...
        f.write(f"Best Fitness (-total_response_time): {best_fitness:.4f}\n")

        f.write(f"Best Fitness (Jain Fairness Index_2): {F_jain_2:.4f}\n")
