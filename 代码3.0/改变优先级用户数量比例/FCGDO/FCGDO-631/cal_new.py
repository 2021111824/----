...
def calculate_weighted_jain_index(individual, n, m_edge, m_cloud, weights, t_delay_e, t_delay_c, user_data,
                                  R_bandwidth, p_user, P_allocation):
    """
    计算加权Jain公平性指数
    """
    ...
    weighted_response_times = compute_weighted_response_times(n, server_indices, t_delay_e_sliced, t_delay_c_sliced,
                                                              user_data, user_bandwidth, p_user_sliced,
                                                              P_allocation_sliced, weights)
...
# 计算加权响应时间
def compute_weighted_response_times(n, m_edge, server_indices, t_delay_e_sliced, t_delay_c_sliced, user_data, user_bandwidth,
                                    p_user_sliced, P_allocation_sliced, weights):
    ...
    return weighted_response_times
...
def compute_response_time(t_delay_e, t_delay_c, is_edge, user_data, user_bandwidth, p_user, P_allocation):
    ...
    响应时间计算
    """
    return t_delay_e + (user_data / user_bandwidth) * 1000 + (
                p_user / (P_allocation if is_edge else (P_allocation * 2)))
