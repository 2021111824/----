...
def initialize_population(n, m_edge, m_cloud, Population, priorities, R_bandwidth, cost_edge, cost_cloud, max_cost,
                          T_max, p_user, p_m, r_m, R_edge, t_delay_e, t_delay_c, user_data, P_allocation, P_cloud):
    """
        ...
        while not valid_individual:
            ...
            for i in range(n):  # 遍历每个用户
                ...
                while not assigned:  # 为每个用户分配一个服务器，直到满足资源约束
                    ...
                    # 检查该服务器是否资源满足限制
                    if math.ceil((server_compute_capability[random_server_idx] + P_allocation[i]) / p_m) * r_m <= R_edge[random_server_idx]:
                        # 如果满足 CPU、内存和带宽约束，分配用户到服务器
                        individual[i, random_server_idx] = 1
                        server_compute_capability[random_server_idx] += P_allocation[i]
...
def compute_fitness(individual, n, m_edge, m_cloud, user_data, weights,
                    R_bandwidth, t_delay_e, t_delay_c, p_user, P_allocation, P_cloud):
    """
    计算适应度值，综合考虑公平性和约束条件。
    """
    # 根据分配情况计算每个用户的带宽
...
def mutate(individual, m_edge, m_cloud, P_mutation):
    """
    对个体进行变异，变异时随机选择一个服务器，不考虑优先级。
    """
    n_server = m_edge + m_cloud  # 总服务器数，包含边缘和云服务器
    for i in range(len(individual)):
        if random.random() < P_mutation:  # 以概率 P_mutation 发生变异
            # 随机选择一个服务器
            server_idx = random.randint(0, n_server - 1)  # 随机选择一个服务器
...
def genetic_algorithm(n, m_edge, m_cloud, priorities, weights, R_bandwidth, cost_edge, cost_cloud,
                      ...
                      t_delay_e, t_delay_c, user_data, P_allocation, P_cloud):
    ...
    for g in tqdm(range(G_max), desc="Running Genetic Algorithm"):
        ...
        for idx, ind in enumerate(new_population[1:]):  # 对新种群进行变异
            mutate(ind, m_edge, m_cloud, P_mutation)
        ...
        for idx, ind in enumerate(new_population):
            if not check_constraints(ind, n, m_edge, m_cloud, priorities, R_bandwidth,
                                     cost_edge, cost_cloud, max_cost, T_max, t_delay_e, t_delay_c,
                                     user_data, p_user, P_allocation, p_m, r_m, R_edge, P_cloud):
                new_population[idx] = repair_individual(ind, n, m_edge, m_cloud, user_data, R_bandwidth, priorities,
                                                        T_max,
                                                        p_user, P_allocation, t_delay_e, t_delay_c, p_m, r_m,
                                                        R_edge, P_cloud)

        # 遗传算法补充种群逻辑
        while len(new_population) < 10:
            base_individual = random.choice(population)  # 随机选择现有个体
            new_individual = mutate(base_individual.copy(), m_edge, m_cloud, P_mutation)
