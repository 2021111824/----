...
def initialize_topology():
    ...
    # 用户请求服务的计算单位量(CU)
    p_user = np.random.uniform(120, 150, n)
    ...
    # 各优先级用户的最大响应时间
    T_max = {
        1: 200,  # 优先级 1 用户最大允许响应时间 (ms)
        2: 150,  # 优先级 2 用户最大允许响应时间 (ms)
        3: 100,  # 优先级 3 用户最大允许响应时间 (ms)
    }
    ...
    # 传播时延初始化
    # 用户与边缘服务器之间的延迟矩阵
    t_delay_e = np.random.uniform(1, 5, (n, m_edge + m_cloud))
    t_delay_c = np.random.uniform(30, 40, n)
    ...
    R_edge = np.concatenate([
        np.random.uniform(2500, 3000, m_edge),  # 边缘服务器的可用计算资源(RU)
