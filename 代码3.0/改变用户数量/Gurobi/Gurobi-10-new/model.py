...
def create_model():
    ...
    model.setParam('MIPGap', 0.05)  # 最优性差距容忍度
    ...
    for i in range(n):
        for j in range(m_edge + m_cloud):
            ...
            response_time = compute_response_time(
                ...
                p_user[i], P_allocation[i], P_cloud
            )
    ...
    for j in range(m_edge):
        # 计算每个服务器上的计算能力需求
        server_compute_capability = gp.quicksum(P_allocation[i] * x[i, j] for i in range(n))  # 每个服务器上的计算能力需求
    ...
    for level in np.unique(priorities):
        ...
        model.addConstr(
            gp.quicksum(
                compute_response_time(t_delay_e[i][j], t_delay_c[i], j < m_edge, user_data[i], user_bandwidth[i], p_user[i], P_allocation[i], P_cloud) * x[i, j]
    ...
    return model, x, individual, user_positions, priorities, weights, server_positions, R_bandwidth, \
        ...
        T_max, max_cost, user_data, t_delay_e, t_delay_c, p_user, P_allocation, p_m, r_m, R_edge, P_cloud
...
def solve_model():
    ...
    model, x, individual, user_positions, priorities, weights, server_positions, R_bandwidth, \
        cost_edge, cost_cloud,  n, m_edge, m_cloud, \
        T_max, max_cost, user_data, t_delay_e, t_delay_c, p_user, P_allocation, p_m, r_m, R_edge, P_cloud = create_model()
    ...
    if model.status == GRB.OPTIMAL:
        ...
        for u in range(n):
            ...
            response_time = compute_response_time(
                ...
                user_data[u], user_bandwidth[u], p_user[u], P_allocation[u], P_cloud
            )
        ...
        for i in range(len(user_positions)):
            ...
            server_compute_capability[server_idx] += P_allocation[i]  # 服务器上的用户计算能力需求
