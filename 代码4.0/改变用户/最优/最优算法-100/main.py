...
from optimal_algorithm import optimal_algorithm, calculate_weighted_jain_index
...
if __name__ == "__main__":
    ...
    # ========== 运行最优算法 ==========
    result = optimal_algorithm(n, m_edge, m_cloud, priorities, weights, cost_edge, cost_cloud, max_cost, T_max,
    ...
    if result is None:
        ...
        for i in range(len(user_positions)):
            ...
            response_time = compute_response_time(t_delay_e[i][server_idx], t_delay_c[i], is_edge, user_data[i], user_bandwidth[i], p_user[i], P_allocation[i], P_cloud)
