...
import random
import numpy as np
from calculations import calculate_edge_server_allocation
...
def initialize_topology():
    ...
    # 85%用户在中心区域（靠近边缘服务器）
    center_users = np.random.choice(np.arange(n), size=int(n * 0.85), replace=False)
    ...
    # 用户优先级初始
    priorities = np.random.choice([1, 2, 3], size=n, p=[0.3, 0.4, 0.3])
    ...
    # 用户请求服务数据大小初始化(Mbps)
    data_in = np.random.uniform(0.1, 0.2, n)
    data_out = np.random.uniform(5, 30, n)
    user_data = (data_in + data_out)

    # 用户请求服务的计算单位量(CU)
    p_user = np.random.uniform(1500, 2000, n)
    ...
    R_edge = np.concatenate([
        np.random.uniform(500, 600, m_edge),  # 边缘服务器的可用计算资源(RU)
        np.full(m_cloud, 200000)  # 云服务器计算资源丰富
    ])

    # 只保留边缘服务器的资源
    R_edge_full = R_edge[:m_edge]  # 取前 m_edge 个元素，确保只计算边缘服务器
    ...
    # 边缘服务器的总计算能力初始化
    # 计算每个边缘服务器上可以部署的服务实例数
    total_available_computing_resources = np.sum(R_edge_full)  # 所有边缘服务器的计算资源总和

    # 使用修改后的函数来计算 P_allocation
    P_allocation = calculate_edge_server_allocation(user_positions, weights, p_user, total_available_computing_resources)
    print(P_allocation)
