...
def initialize_topology():
    ...
    # 用户请求服务的计算单位量(CU)
    p_user = np.random.uniform(10, 50, n)

    # 用户的计算能力分配量(CU/ms)
    P_allocation = weights * p_user * 0.03

    # 各优先级用户的最大响应时间
    T_max = {
        1: 400,  # 优先级 1 用户最大允许响应时间 (ms)
        2: 250,  # 优先级 2 用户最大允许响应时间 (ms)
        3: 120,  # 优先级 3 用户最大允许响应时间 (ms)
    }
    ...
    # 传播时延初始化
    # 用户与边缘服务器之间的延迟矩阵
    t_delay_e = np.random.uniform(3, 10, (n, m_edge + m_cloud))
    t_delay_c = np.random.uniform(20, 40, n)

    # 服务器总可用带宽初始化
    R_bandwidth = np.concatenate([
        np.random.uniform(50000, 100000, m_edge),  # 边缘服务器总可用带宽为 5-10Gbps
        np.full(m_cloud, 500)  # 云服务器可用带宽为 500Mbps
    ])
    print(R_bandwidth)

    # 服务器可用计算资源初始化
    R_edge = np.concatenate([
        np.random.uniform(500, 1000, m_edge),  # 边缘服务器的可用计算资源(RU)
