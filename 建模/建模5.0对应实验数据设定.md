...
def initialize_topology():
    ...
    # 用户的计算能力分配量(CU/ms)
    P_allocation = weights * p_user * 0.1

    # 各优先级用户的最大响应时间
    T_max = {
        1: 40,  # 优先级 1 用户最大允许响应时间 (ms)
        2: 45,  # 优先级 2 用户最大允许响应时间 (ms)
    ...
    R_bandwidth = np.concatenate([
        np.random.uniform(200000, 300000, m_edge),  # 边缘服务器总可用带宽为 5-10Gbps
    ...
    R_edge = np.concatenate([
        np.random.uniform(40, 60, m_edge),  # 边缘服务器的可用计算资源(RU)
