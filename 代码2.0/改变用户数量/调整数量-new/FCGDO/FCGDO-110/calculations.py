...
def compute_response_time(t_delay_e, t_delay_c, is_edge, user_data, user_bandwidth, p_user, P_allocation):
    ...
    if is_edge:
        ...
        print("总时延：" + str(t_delay_c + (user_data / user_bandwidth) * 1000 + (p_user / (P_allocation * 50))) + "ms")
        return t_delay_c + (user_data / user_bandwidth) * 1000 + (p_user / (P_allocation * 50))  # 云服务器响应时间
