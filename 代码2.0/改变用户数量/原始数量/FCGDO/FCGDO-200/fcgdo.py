...
def greedy_algorithm(n, m_edge, m_cloud, priorities, weights, cost_edge, cost_cloud, max_cost, T_max,
    ...
    # 优先对 bad_connections 中的请求进行迁移优化
    individual, current_jain = migrate_requests(individual, n, m_edge, m_cloud, weights, priorities, R_bandwidth,
                                                cost_edge, cost_cloud, max_cost, T_max, t_delay_e, t_delay_c,
                                                user_data, p_user, P_allocation, p_m, r_m, R_edge, bad_connections)

    # 再对 bad_connections 之外的请求进行迁移优化
    all_users = set(range(n))
    individual, final_jain = migrate_requests(individual, n, m_edge, m_cloud, weights, priorities, R_bandwidth,
                                              cost_edge, cost_cloud, max_cost, T_max, t_delay_e, t_delay_c,
                                              user_data, p_user, P_allocation, p_m, r_m, R_edge, all_users)
...
def migrate_requests(individual, n, m_edge, m_cloud, weights, priorities, R_bandwidth, cost_edge, cost_cloud, max_cost,
                     T_max, t_delay_e, t_delay_c, user_data, p_user, P_allocation, p_m, r_m, R_edge, user_indices):
