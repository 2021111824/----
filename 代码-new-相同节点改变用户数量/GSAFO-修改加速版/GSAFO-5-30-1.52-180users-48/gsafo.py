...
# 单个贪心算法运行
def single_greedy_run(args):
    n, m_edge, m_cloud, priorities, weights, cost_edge, cost_cloud, max_cost, T_max, \
    R_bandwidth, t_delay_e, t_delay_c, p_m, r_m, R_edge, user_data, p_user, P_allocation = args
    ...
    while not valid_individual:
        ...
        for i in sorted_indices:
            ...
            for server_idx in potential_servers:
                ...
                if jain_index > best_jain_temp:
                    best_jain_temp = jain_index
    ...
    jain_index = calculate_weighted_jain_index(individual, n, m_edge, m_cloud, weights,
                                               t_delay_e, t_delay_c, user_data, R_bandwidth,
                                               p_user, P_allocation)
    return individual, jain_index
...
def greedy_algorithm(n, m_edge, m_cloud, priorities, weights, cost_edge, cost_cloud, max_cost, T_max,
    ...
    """
    num_runs = 20  # 这里设置贪心算法运行的次数，你可以根据需要调整
    args = [(n, m_edge, m_cloud, priorities, weights, cost_edge, cost_cloud, max_cost, T_max,
             R_bandwidth, t_delay_e, t_delay_c, p_m, r_m, R_edge, user_data, p_user, P_allocation) for _ in range(num_runs)]

    with Pool() as pool:
        results = pool.map(single_greedy_run, args)

    best_jain = -1
    best_individual = None
    for individual, jain_index in results:
        if jain_index > best_jain:
            best_jain = jain_index
            best_individual = individual.copy()

    # 对找到的最好的贪心结果进行模拟退火
    best_individual, best_jain, best_response_times = sa_local_optimization(best_individual, n, m_edge, m_cloud, priorities,
                                                                            weights,
...
def sa_local_optimization(individual, n, m_edge, m_cloud, priorities, weights,
    ...
    for _ in range(max_iters):
        ...
        # 计算新 Jain 指数
        new_jain = calculate_weighted_jain_index(new_individual, n, m_edge, m_cloud, weights, t_delay_e, t_delay_c,
                                                 user_data, R_bandwidth, p_user, P_allocation)
