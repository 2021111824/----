...
from gsafo import greedy_algorithm
...
# 修改 run_greedy_algorithm 函数，接受所需参数
def run_greedy_algorithm(n, m_edge, m_cloud, priorities, weights, cost_edge, cost_cloud, max_cost, T_max,
                         R_bandwidth, t_delay_e, t_delay_c, p_m, r_m, R_edge, user_data, p_user, P_allocation):
    return greedy_algorithm(n, m_edge, m_cloud, priorities, weights, cost_edge, cost_cloud, max_cost, T_max,
                            R_bandwidth, t_delay_e, t_delay_c, p_m, r_m, R_edge, user_data, p_user, P_allocation)


# 定义一个普通函数来替代 lambda 函数
def unpack_and_run(args):
    return run_greedy_algorithm(*args)
...
if __name__ == "__main__":
    ...
    # ========== 运行贪心算法 ==========
    args_list = [(n, m_edge, m_cloud, priorities, weights, cost_edge, cost_cloud, max_cost, T_max,
                  R_bandwidth, t_delay_e, t_delay_c, p_m, r_m, R_edge, user_data, p_user, P_allocation) for _ in
                 range(20)]

    with ProcessPoolExecutor() as executor:
        results = list(executor.map(unpack_and_run, args_list))

    best_final_result = None
    best_final_jain = -1
    best_final_response_times = None
    for result, best_jain, best_response_times in results:
        if best_jain > best_final_jain:
            best_final_jain = best_jain
            print(best_final_jain)
    ...
    if best_final_result is None:
        ...
        np.add.at(server_compute_capability, server_indices, p_user)
        ...
        simulation_result_file_content = [
            "===== Simulation Results =====",
            f"Best Fitness (Jain Fairness Index): {F_jain:.4f}",
            f"Average Response Time of Best Solution: {avg_response_time:.2f} ms\n",
